...
def build_fsd_module(config: ModuleBuildConfig, script_file: str):
    ...
    for i, match in enumerate(mermaid_blocks):
        ...
        if png_path and 'erDiagram' in code:
            try:
                from fsd_build import ensure_png_min_width
    ...
    if has_swimlane:
        print('   [Swimlane] terdeteksi — lebar diagram max {:.0f} cm'.format(
            config.swimlane_image_width_cm))
    if config.portrait_image_max_width_cm:
        print('   [Mobile UI] screenshot portrait max {:.1f} cm lebar'.format(
            config.portrait_image_max_width_cm))
    ...
    postprocess_docx(
        ...
        portrait_max_width_cm=config.portrait_image_max_width_cm,
        portrait_max_height_cm=config.portrait_image_max_height_cm,
    )
