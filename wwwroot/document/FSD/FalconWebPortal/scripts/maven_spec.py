...
def chapter_erd_penjualan() -> str:
    """Bab 7 fisik MAVEN — sumber kebenaran: FSD_Falcon_Web_Penjualan_v1.0.md."""
    import os
    md_path = os.path.join(
        os.path.dirname(os.path.abspath(__file__)),
        "..",
        "source",
        "FSD_Falcon_Web_Penjualan_v1.0.md",
    )
    with open(md_path, encoding="utf-8") as f:
        body = f.read()
    a = body.find("## 7. Struktur Data & ERD")
    b = body.find("\n## 8. ")
    if a < 0 or b < 0:
        raise RuntimeError("Bab 7 tidak ditemukan di FSD Penjualan source")
