...
function validatePayload(body) {
    ...
    return {
        ...
        sections,
        refreshPolicy: body.refreshPolicy === 'full' ? 'full' : 'smart',
        refreshScreenshots: body.refreshScreenshots === true,
    };
}
...
async function handlePostGenerate(req, body) {
    ...
    if (!rate.allowed) {
        return {
            status: 429,
            body: {
                error: `Batas generate: max ${process.env.FSD_RATE_LIMIT_PER_HOUR || 20} job/jam. Coba lagi nanti.`,
