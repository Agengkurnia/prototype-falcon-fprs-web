...
async function callGeminiFlow(systemPrompt, userPrompt, geminiKey, model, attempt = 0) {
    ...
    if (RETRYABLE_STATUS.has(res.status) && attempt < MAX_GEMINI_ATTEMPTS) {
    ...
    if (!res.ok) {
        ...
        const fallback = process.env.FSD_GEMINI_FALLBACK_MODEL;
        if (fallback && fallback !== model && RETRYABLE_STATUS.has(res.status)) {
            return callGeminiFlow(systemPrompt, userPrompt, geminiKey, fallback, 0);
        }
        if (res.status === 429) throw new Error('Quota Gemini habis');
        if (res.status === 503) {
            throw new Error('Gemini sibuk (503) — coba lagi beberapa menit atau set FSD_GEMINI_FALLBACK_MODEL=gemini-2.5-flash-lite');
        }
