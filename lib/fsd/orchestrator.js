...
const { buildPortalDiagramBlocks } = require('./render-diagrams');
...
async function runGenerate(params) {
    ...
    const diagramBlocks = mode === 'full'
        ? await buildPortalDiagramBlocks(sections || [])
        : [];

    const merged = mergeDocuments(moduleDocs, mode, diagramBlocks);
    const buffer = await buildDocxFromBlocks(merged.blocks, ROOT, getStaticBaseUrl());
