...
def wait_ready(page, mod_id=None):
    try:
    ...
    try:
        ...
        pass
    if mod_id and (mod_id.startswith('kunjungan-') or mod_id.startswith('master-')):
        try:
            page.wait_for_selector(
                '#tbl tbody tr, #tblBody tr, .dataTables_wrapper, #app-content',
                timeout=12000,
            )
        except Exception:
            pass
    if mod_id and mod_id.startswith('kunjungan-'):
        time.sleep(2.5)
    else:
        time.sleep(0.8)


def use_full_page(mod: dict, shot_name: str) -> bool:
    """Full-page capture untuk Master Data dan halaman index/form panjang."""
    mid = mod.get('id', '')
    if mod.get('group') == 'masterData' or mid.startswith('master-'):
        return True
    if mid == 'dashboard':
        return True
    low = shot_name.lower()
    if any(x in low for x in ('_index', '_add', '_edit', '_modal', 'informasi', 'geografis', 'rute', 'faktur', 'canvassing', 'motoris')):
        return True
    return False


def take_shot(page, out_dir: str, shot_name: str, mod: dict) -> None:
    opts = {'path': os.path.join(out_dir, shot_name)}
    if use_full_page(mod, shot_name):
        opts['full_page'] = True
...
def capture_page_module(page, mod, base_url, out_dir, shots):
    ...
    wait_ready(page, mod.get('id'))
    if shots:
        take_shot(page, out_dir, shots[0], mod)
    ...
    if add_shot:
        ...
        wait_ready(page, mod.get('id'))
        take_shot(page, out_dir, add_shot, mod)
    ...
    if edit_shot:
        ...
        if edit_href:
            ...
            wait_ready(page, mod.get('id'))
            take_shot(page, out_dir, edit_shot, mod)
            captured.append(edit_shot)
        else:
            edit_btn = page.locator('.btn-action-edit, button.btn-action-edit').first
            if edit_btn.count():
                edit_btn.click(timeout=5000)
                time.sleep(0.8)
                if 'add.html' in page.url or page.locator('#modalForm.show, .modal.show').count():
                    take_shot(page, out_dir, edit_shot, mod)
                    captured.append(edit_shot)
    ...
    if val_shot:
        ...
        wait_ready(page, mod.get('id'))
        submit = page.locator(
            'button[type="submit"], button.btn-success[onclick*="save"], #formProduk button[type="submit"]'
        ).first
        if submit.count():
            submit.click(timeout=5000)
            time.sleep(1.0)
            take_shot(page, out_dir, val_shot, mod)
    ...
    if del_shot:
        ...
        if del_btn.count():
            ...
            time.sleep(0.8)
            take_shot(page, out_dir, del_shot, mod)
...
def capture_modal_module(page, mod, base_url, out_dir, shots):
    ...
    if btn.count():
        ...
        time.sleep(0.6)
        modal = shot_by_pattern(shots, 'modal_tambah', 'modal')
        if modal:
            take_shot(page, out_dir, modal, mod)
            captured.append(modal)

    edit_shot = shot_by_pattern(shots, 'modal_edit', '_edit')
    if edit_shot and edit_shot not in captured:
        page.goto(url, wait_until='domcontentloaded', timeout=30000)
        wait_ready(page, mod.get('id'))
        edit_btn = page.locator('.btn-action-edit, button.btn-action-edit').first
        if edit_btn.count():
